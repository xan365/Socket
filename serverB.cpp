...
#define PORTB 31997 //the UDP port of server B
...
int main()
{
    cout << "The server B is up and running using UDP on port " << PORTB << endl;
...
// first visit , build graph
void build_graph()
{
    int count = -1;
    string country;

    //read data2.txt and build graph
...
string recommend(string country_name, string id)
{
    ...
    if (all_users.count(user_id) == 0)
    {
        ...
        cout << "The server B has sent “User " << user_id << " not found” to Main Server" << endl;
        return "No_such_user";
    }
    //find result
    unordered_map<int, set<int>> curr_map = country_map[country_name]; //  get the map with current country

    cout << "The server B is searching possible friends for " << user_id << " ..." << endl;
    ...
    cout << "The server B has sent the result " << to_string(res) << " to Main Server" << endl;
